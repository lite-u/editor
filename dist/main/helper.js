...
exports.generateElementsDetectArea = generateElementsDetectArea;
exports.generateSelectedBoundingElement = generateSelectedBoundingElement;
exports.generateTransformHandles = generateTransformHandles;
exports.generateAnchorAndPath = generateAnchorAndPath;
const helper_1 = require("~/services/tool/resize/helper");
const rectangle_1 = __importDefault(require("~/elements/rectangle/rectangle"));
const rectangle_2 = __importDefault(require("~/elements/rectangle/rectangle"));
...
const ellipse_2 = __importDefault(require("~/elements/ellipse/ellipse"));
const index_1 = require("~/index");
const helper_2 = require("~/services/tool/selector/helper");
const lineSegment_1 = __importDefault(require("~/elements/lines/lineSegment"));
function generateElementsDetectArea() {
    ...
    visibleElements.forEach((ele) => {
        ...
        if (renderCenterPoint) {
            ...
            centerPoint = idSet.size === 1
                ? ellipse_2.default.create((0, index_1.nid)(), ele.cx, ele.cy, pointLen)
                : rectangle_1.default.create((0, index_1.nid)(), ele.cx, ele.cy, pointLen);
            if (isSelected) {
...
function generateSelectedBoundingElement() {
    ...
    const selectedOutlineElement = new rectangle_1.default(Object.assign(Object.assign({ id: 'selected-elements-outline', layer: maxLayer, show: !specialLineSeg, type: 'rectangle' }, rect), { rotation: applyRotation, stroke: Object.assign(Object.assign({}, defaultProps_1.DEFAULT_STROKE), { weight: 1 * ratio, color: boxColor }) }));
    overlayHost.append(selectedOutlineElement);
    return selectedOutlineElement;
}
function generateTransformHandles(ele, specialLineSeg = false) {
    const { world } = this;
    const boxColor = '#4f80ff';
    const { scale, dpr } = world;
    const ratio = dpr / scale;
    ...
    const handleRotateMouseEnter = () => {
        const mouseCurrentRotation = (0, helper_2.getRotateAngle)({ x: cx, y: cy }, this.interaction.mouseWorldCurrent);
    ...
    arr.map(({ dx, dy, name }) => {
        if (specialLineSeg && name !== 't' && name !== 'b')
            return;
        const { x, y } = (0, geometry_1.rotatePointAroundPoint)(cx + dx * width, cy + dy * height, cx, cy, rotation);
        const resizeEle = rectangle_2.default.create('handle-resize-' + name, x, y, resizeLen);
        const rotateEle = ellipse_1.default.create('handle-rotate-' + name, x, y, rotateRadius);
...
function generateAnchorAndPath() {
    ...
    elements.forEach(ele => {
        ...
        points.forEach((point, index) => {
            ...
            const anchorPoint = rectangle_1.default.create(point.id, aPX, aPY, pointLen);
            ...
            if (point.cp1) {
                ...
                cp1 = ellipse_2.default.create(point.id + '-cp1', cPX, cPY, pointLen);
                cp1.layer = maxLayer + 1;
                cp1.fill.enabled = true;
                cp1.fill.color = boxColor;
                cp1.stroke.enabled = false;
                line1 = lineSegment_1.default.create(point.id + '-cp1-line', cPX, cPY, aPX, aPY);
           ...
           */
            ...
            if (point.cp2) {
                ...
                cp2 = ellipse_2.default.create(point.id + '-cp2', cPX, cPY, pointLen);
                line2 = lineSegment_1.default.create(point.id + '-line-cp2', cPX, cPY, aPX, aPY);
